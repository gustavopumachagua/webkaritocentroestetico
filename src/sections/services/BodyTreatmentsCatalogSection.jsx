...
const INITIAL_VISIBLE_TREATMENTS = 6;
...
function TreatmentCard({ item, index }) {
  ...
  return (
    <Tag
      ...
      className="body-card group relative flex flex-col overflow-hidden rounded-[1.75rem] border border-white/8"
    ...
    >
      <div className="relative overflow-hidden">
        <OptimizedImage
          src={item.imageSrc}
          alt={item.imageAlt}
          wrapperClassName="aspect-[6/5] w-full sm:aspect-[4/5]"
        ...
        />
        {item.tag && (
          <div className="absolute left-4 bottom-4 flex items-center gap-1.5 rounded-full border border-white/20 bg-black/30 px-3 py-1.5 backdrop-blur-sm">
        ...
        {item.href && (
          <div className="mt-6 flex items-center justify-between border-t border-white/8 pt-5">
            <span className="text-sm font-semibold text-[var(--color-accent)]">
              {uiText.bodyTreatmentsCatalog.viewMore}
            </span>
            <span
      ...
      <style>{`
        .body-card {
          ...
          animation: bodyCardIn 0.55s cubic-bezier(0.22,1,0.36,1) both;
        }
        .body-card:hover {
          box-shadow: 0 18px 36px rgba(0,0,0,0.28), 0 30px 70px rgba(0,0,0,0.2);
          transform: translateY(-5px);
        }
        @keyframes bodyCardIn {
...
export default function BodyTreatmentsCatalogSection({
  items = bodyTreatmentsPage.treatmentsSection.items,
}) {
  const { treatmentsSection } = bodyTreatmentsPage;
  const [isExpanded, setIsExpanded] = useState(false);
  const visibleItems = isExpanded
    ? items
    : items.slice(0, INITIAL_VISIBLE_TREATMENTS);
  return (
    <section className="relative overflow-hidden border-y border-white/5 bg-[#091019] py-20 sm:py-28 lg:py-32">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div
          className="absolute left-1/2 top-0 h-[500px] w-[700px] -translate-x-1/2 -translate-y-1/3 rounded-full opacity-50"
          style={{
            background:
        ...
        />
        <div
          className="absolute -left-24 bottom-0 h-[360px] w-[360px] rounded-full opacity-40"
          style={{
            background:
        ...
        />
        <div
          className="absolute -right-24 top-1/3 h-[300px] w-[300px] rounded-full opacity-30"
          style={{
            background:
      ...
      <Container className="relative z-10">
        <div className="catalog-header mx-auto max-w-3xl text-center">
          ...
          <p className="mt-5 text-base leading-8 text-[var(--color-ink-soft)] sm:text-lg">
            {treatmentsSection.description ??
              uiText.bodyTreatmentsCatalog.fallbackDescription}
          </p>
        </div>
        ...
        {items.length > INITIAL_VISIBLE_TREATMENTS ? (
