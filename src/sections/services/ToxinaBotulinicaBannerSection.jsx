...
export default function ToxinaBotulinicaBannerSection() {
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#081019_0%,#0d141d_100%)] pb-16 pt-28 sm:pb-20 sm:pt-36 lg:pb-24 lg:pt-40">
      <h1 className="sr-only">{toxinaBotulinicaPage.title}</h1>
      ...
      <Container className="relative z-10 max-w-[104rem]">
        <div
          className="toxina-frame relative rounded-[2rem] p-[3px]"
          style={{
            background:
        ...
        >
          <div className="overflow-hidden rounded-[1.8rem] bg-[#091019] p-1.5">
            <div className="group relative overflow-hidden rounded-[1.5rem]">
              <img
                src={toxinaBotulinicaPage.bannerSrc}
                alt={toxinaBotulinicaPage.bannerAlt}
              ...
              />
              <div className="toxina-chip absolute bottom-5 left-5 flex items-center gap-3 rounded-2xl border border-white/12 bg-[rgba(8,11,16,0.78)] px-5 py-3 backdrop-blur-md">
                ...
                <span className="text-sm font-bold tracking-wide text-[var(--color-accent)]">
                  {toxinaBotulinicaPage.bannerLabel ??
                    toxinaBotulinicaPage.title}
                </span>
              </div>
            </div>
          </div>
      ...
      <style>{`
        @keyframes toxinaEyebrowIn {
          from { opacity: 0; transform: translateX(-12px); }
          to   { opacity: 1; transform: translateX(0); }
        }
        @keyframes toxinaFrameIn {
          from { opacity: 0; transform: translateY(20px) scale(0.98); }
          to   { opacity: 1; transform: translateY(0) scale(1); }
        }
        @keyframes toxinaChipIn {
          from { opacity: 0; transform: translateY(8px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        @keyframes toxinaMetaIn {
          from { opacity: 0; }
          to   { opacity: 1; }
        }
        .toxina-eyebrow { animation: toxinaEyebrowIn 0.5s  cubic-bezier(0.22,1,0.36,1) both; }
        .toxina-frame   { animation: toxinaFrameIn   0.7s  cubic-bezier(0.22,1,0.36,1) 0.1s both; }
        .toxina-chip    { animation: toxinaChipIn    0.5s  cubic-bezier(0.22,1,0.36,1) 0.55s both; }
        .toxina-badge   { animation: toxinaChipIn    0.5s  cubic-bezier(0.22,1,0.36,1) 0.65s both; }
        .toxina-meta    { animation: toxinaMetaIn    0.6s  ease 0.8s both; }
      `}</style>
    </section>
  );
}
