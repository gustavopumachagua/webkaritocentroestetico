...
export default function DeepCleanseBannerSection() {
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#081019_0%,#0d141d_100%)] pb-16 pt-28 sm:pb-20 sm:pt-36 lg:pb-24 lg:pt-40">
      <h1 className="sr-only">{deepCleansePage.title}</h1>
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div
          className="absolute -left-32 top-8 h-[420px] w-[420px] rounded-full opacity-70"
          style={{
            background:
        ...
        />
        <div
          className="absolute -right-32 bottom-0 h-[380px] w-[380px] rounded-full opacity-50"
          style={{
            background:
      ...
      <Container className="relative z-10">
        <div
          className="banner-frame relative rounded-[2rem] p-[3px]"
          style={{
            background:
        ...
        >
          <div className="overflow-hidden rounded-[1.8rem] bg-[#091019] p-1.5">
            <div className="group relative overflow-hidden rounded-[1.5rem]">
              <img
                src={deepCleansePage.bannerSrc}
                alt={deepCleansePage.bannerAlt}
              ...
              />
              <div className="absolute bottom-5 left-5 flex items-center gap-3 rounded-2xl border border-white/12 bg-[rgba(8,11,16,0.78)] px-5 py-3 backdrop-blur-md banner-chip">
                ...
                <span className="text-sm font-bold tracking-wide text-[var(--color-accent)]">
                  {deepCleansePage.bannerLabel ?? deepCleansePage.title}
                </span>
              </div>
            </div>
          </div>
      ...
      <style>{`
        @keyframes bannerEyebrowIn {
          from { opacity: 0; transform: translateX(-12px); }
          to   { opacity: 1; transform: translateX(0); }
        }
        @keyframes bannerFrameIn {
          from { opacity: 0; transform: translateY(20px) scale(0.98); }
          to   { opacity: 1; transform: translateY(0)   scale(1); }
        }
        @keyframes bannerChipIn {
          from { opacity: 0; transform: translateY(8px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        @keyframes bannerMetaIn {
          from { opacity: 0; }
          to   { opacity: 1; }
        }
        .banner-eyebrow { animation: bannerEyebrowIn 0.5s cubic-bezier(0.22,1,0.36,1) both; }
        .banner-frame   { animation: bannerFrameIn   0.7s cubic-bezier(0.22,1,0.36,1) 0.1s both; }
        .banner-chip    { animation: bannerChipIn    0.5s cubic-bezier(0.22,1,0.36,1) 0.55s both; }
        .banner-badge   { animation: bannerChipIn    0.5s cubic-bezier(0.22,1,0.36,1) 0.65s both; }
        .banner-meta    { animation: bannerMetaIn    0.6s ease 0.8s both; }
      `}</style>
    </section>
  );
}
