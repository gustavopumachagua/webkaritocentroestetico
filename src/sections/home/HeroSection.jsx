...
export default function HeroSection() {
  ...
  return (
    <section
      id="inicio"
      className="relative isolate flex h-screen h-[100svh] min-h-[100svh] flex-col overflow-hidden bg-[#0c0806] text-white"
    >
      <h1 className="sr-only">{site.brandMark}</h1>
      {slides.map((slide, i) => (
        <div
          key={slide.id}
      ...
      <div className="absolute inset-x-0 bottom-0 h-44 bg-gradient-to-t from-[rgba(12,8,6,0.74)] to-transparent" />
      <div className="absolute bottom-7 left-4 z-20 flex items-center gap-2 sm:left-6">
        <button
          aria-label={uiText.hero.previousSlide}
          onClick={() => stepSlide(-1)}
          className="flex h-10 w-10 items-center justify-center border border-white/25 bg-white/8 text-white/70 transition hover:border-white/55 hover:bg-white/14 hover:text-white"
        >
          <svg
            className="h-3 w-3"
            fill="none"
