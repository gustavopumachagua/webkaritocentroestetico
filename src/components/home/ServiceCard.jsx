...
export default function ServiceCard({ service, image, itemNumber }) {
  ...
  return (
    <Component
      {...extraProps}
      className="group relative aspect-[4/5] cursor-pointer overflow-hidden rounded-lg border border-white/8 bg-[#0d121a]
        shadow-[0_12px_36px_rgba(0,0,0,0.26)]
        transition-all duration-300
        hover:-translate-y-1 hover:shadow-[0_24px_64px_rgba(0,0,0,0.4)]"
    >
      <OptimizedImage
        src={image}
        alt={service.title}
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 px-[22px] pb-[22px]">
        <h3
          className="mb-3 font-[var(--font-display)] text-[1.75rem] font-semibold leading-[1.05] text-white sm:text-[2rem]"
        >
          {service.title}
        </h3>
      ...
      <div className="pointer-events-none absolute inset-0 rounded-lg ring-1 ring-white/6 transition duration-300 group-hover:ring-[var(--color-primary)]/28" />
    </Component>
  );
}
