...
export default function PromotionCard({ promotion, image, onClick }) {
  ...
  return (
    <Component
      {...actionProps}
      className="group relative flex h-full w-full flex-col overflow-hidden rounded-lg border border-white/8 bg-[linear-gradient(180deg,rgba(20,27,37,0.96)_0%,rgba(11,16,22,0.98)_100%)] text-left shadow-[0_16px_48px_rgba(0,0,0,0.28)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(0,0,0,0.4)]"
    >
      <div className="relative overflow-hidden">
        <OptimizedImage
          src={image}
          alt={promotion.title}
          wrapperClassName="h-[15rem] w-full sm:h-[17rem]"
      ...
      <div className="flex flex-1 flex-col justify-between gap-4 px-5 py-5 sm:px-6 sm:py-6">
        <div>
          <h3 className="font-[var(--font-display)] text-[1.45rem] font-semibold leading-[1.08] text-[var(--color-ink)] transition duration-200 group-hover:text-[var(--color-accent)] sm:text-[1.7rem]">
            {promotion.title}
          </h3>
