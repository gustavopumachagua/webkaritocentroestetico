...
function LanguageToggle({
  ...
  const items = [
    { code: "es", flag: "🇪🇸", label: uiText.locale.spanish },
    { code: "en", flag: "🇺🇸", label: uiText.locale.english },
  ];
  const buttonClassName = compact
    ? "h-10 w-10 text-[1.08rem]"
    : desktop
      ? "h-9 w-9 text-[1.02rem] xl:h-10 xl:w-10 xl:text-[1.08rem]"
      : "h-10 w-10 text-[1.08rem]";
  const activeButtonClassName = desktop
    ? "border-[var(--color-primary)]/45 bg-[var(--color-primary)]/18 shadow-[0_8px_22px_rgba(160,94,76,0.28)]"
    : "border-[var(--color-primary)]/45 bg-[var(--color-primary)]/18 shadow-[0_8px_22px_rgba(160,94,76,0.26)]";

  return (
    <div
      aria-label={uiText.locale.ariaLabel}
      className="flex w-fit items-center gap-1 rounded-full border border-white/12 bg-black/25 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl"
    >
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={`inline-flex items-center justify-center rounded-full border transition-all duration-300 ${buttonClassName} ${
              isActive
                ? activeButtonClassName
                : "border-transparent bg-transparent hover:border-white/12 hover:bg-white/8"
            }`.trim()}
            aria-pressed={isActive}
            aria-label={item.label}
            title={item.label}
          >
            <span className="leading-none drop-shadow-[0_1px_4px_rgba(0,0,0,0.25)]" aria-hidden>
              {item.flag}
            </span>
          </button>
        );
...
export default function SiteHeader() {
  ...
  const servicesMenu = headerNavigation.find((item) => item.submenu?.length);
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      >
          ...
          <div className="flex h-16 items-center justify-between gap-3 lg:grid lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:gap-4 xl:gap-7">
            ...
            >
              ...
              <div className="min-w-0">
                <span
                  className="block font-[var(--font-display)] leading-tight text-white"
        ...
        {dropdownOpen && servicesMenu?.submenu?.length ? (
          <div
            id="services-desktop-menu"
            className="hidden border-t border-white/8 bg-[rgba(6,9,13,0.98)] shadow-[0_24px_58px_rgba(0,0,0,0.38)] backdrop-blur-2xl lg:block"
          >
            <Container size="xl">
              <div
                role="menu"
                className="mx-auto my-3 w-full max-w-[18.5rem] overflow-hidden rounded-2xl border border-white/10 bg-[rgba(8,13,19,0.96)] p-1.5 shadow-[0_18px_44px_rgba(0,0,0,0.32)]"
              >
                {servicesMenu.submenu.map((sub) => {
                  const Icon = getServiceIcon(sub.href);
                  const subActive = normalizePathname(sub.href) === pathname;
                  ...
                  return (
                    <a
                      ...
                      href={sub.href}
                      role="menuitem"
                      className={`group flex h-11 items-center gap-2.5 rounded-xl px-2.5 py-2 transition hover:bg-white/7 ${subActive ? "bg-white/8" : ""}`.trim()}
                      onClick={(e) => navigate(e, sub.href)}
                    >
                      <span
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border bg-white/[0.04] transition group-hover:border-[var(--color-primary)]/25 group-hover:bg-[var(--color-primary)]/10 group-hover:text-[var(--color-accent)] ${subActive ? "border-[var(--color-primary)]/30 text-[var(--color-accent)]" : "border-white/8 text-[var(--color-accent)]/68"}`.trim()}
                      >
                        <Icon className="text-xs" />
                      </span>
                      <span
                        className={`min-w-0 flex-1 text-[0.8rem] font-medium leading-none transition group-hover:text-white ${subActive ? "text-white" : "text-white/72"}`.trim()}
                      >
                        {sub.label}
                      </span>
                      <FaChevronRight
                        className={`ml-auto text-[0.54rem] transition group-hover:translate-x-0.5 group-hover:text-white/55 ${subActive ? "text-white/50" : "text-white/24"}`.trim()}
                      />
                    </a>
                  );
                })}
