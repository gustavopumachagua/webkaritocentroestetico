...
- Si Cloudinary responde con error para una imagen concreta, el componente vuelve a la imagen local importada.
...
- `src/assets/images/hero/desktop_informacion.png` -> `tu-carpeta-base/hero/desktop_informacion`
...
La subida usa dos capas de optimizacion:

- `CLOUDINARY_LOCAL_OPTIMIZE=true` crea una copia temporal mas liviana antes de enviar la imagen, sin tocar el archivo local original.
- `CLOUDINARY_UPLOAD_TRANSFORMATION` aplica una transformacion entrante en Cloudinary antes de guardar el asset final.

La entrega del sitio usa `VITE_CLOUDINARY_TRANSFORMATION`, normalmente `f_auto,q_auto`, para servir el formato y la calidad mas convenientes al navegador.
