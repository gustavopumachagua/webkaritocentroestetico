...
```text
src/assets/images/hero/desktop_informacion.png
=> public_id: <VITE_CLOUDINARY_BASE_FOLDER>/hero/desktop_informacion
```
...
- Si el asset remoto falla o aun no existe, el componente de imagen usa la imagen local como respaldo para evitar espacios vacios.
- El script de subida aplica `CLOUDINARY_UPLOAD_TRANSFORMATION` como transformacion entrante para reducir peso antes de guardar el original en Cloudinary.
...
# Subida local a Cloudinary. No uses prefijo VITE_ en credenciales privadas.
CLOUDINARY_API_KEY=
CLOUDINARY_API_SECRET=
CLOUDINARY_MAX_UPLOAD_BYTES=10485760
CLOUDINARY_UPLOAD_TRANSFORMATION=c_limit,w_1920,h_1920,q_auto:eco,fl_strip_profile
CLOUDINARY_LOCAL_OPTIMIZE=true
CLOUDINARY_OPTIMIZE_MIN_BYTES=153600
CLOUDINARY_LOCAL_MAX_DIMENSION=1920
...
- `CLOUDINARY_API_KEY` y `CLOUDINARY_API_SECRET`: necesarias para la subida automatizada desde el script local. No llevan prefijo `VITE_`.
- `CLOUDINARY_MAX_UPLOAD_BYTES`: limite local antes de intentar comprimir archivos muy pesados.
- `CLOUDINARY_UPLOAD_TRANSFORMATION`: transformacion entrante aplicada al subir para limitar resolucion, retirar perfiles y ajustar calidad.
- `CLOUDINARY_LOCAL_OPTIMIZE`: crea una copia temporal optimizada antes de subir; no modifica los archivos locales originales.
- `CLOUDINARY_LOCAL_MAX_DIMENSION`: dimension maxima usada en la copia temporal optimizada.
- `CLOUDINARY_LOCAL_JPEG_QUALITY` y `CLOUDINARY_LOCAL_WEBP_QUALITY`: calidad usada para compactar JPG/JPEG y WebP antes de subir.
